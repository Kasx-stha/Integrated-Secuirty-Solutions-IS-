...
class IDS:
    ...
    def _forward_alert_to_snort(self, alert_type: str, details: Dict, packet) -> bool:
        try:
            #success, msg = self.snort_integration.forward_alert(alert_type, details, packet)
            #if success:
             #   self.logger.info(f"Forwarded {alert_type} alert to Snort IPS.")
            #else:
             #   self.logger.warning(f"Failed to forward {alert_type} alert to Snort IPS: {msg}")
            #return success
